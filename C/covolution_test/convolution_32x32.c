...
*/
    ...
    FILE *fp_kernel_output = fopen("c:\\Users\\kingjy79\\Documents\\rlawhdduq1\\C\\covolution_test\\w_out_1s.dat","w");
    for(int i=0; i<150; i++){
        fprintf(fp_kernel_output, "%d \n", (int)(kernel_get[i]));
    }
    fclose(fp_kernel_output);




    FILE *fp_input = fopen("c:\\Users\\kingjy79\\Documents\\rlawhdduq1\\C\\covolution_test\\x_in_1s.dat","r");
    char input_get[1024]; //input_size:32*32
    if(fp_input==NULL){
        printf("Error opening file_input_get");
    }
    else{
        fgets(input_get, 1024, fp_input);
        fclose(fp_input);
    }
...
/*
    ...
    for(int i=0; i<output_height;i++){
        output_matrix[i]=(int*) malloc(sizeof(int)*output_width);
    }
    //배열 초기화
    for(int i=0; i<output_height; i++){
